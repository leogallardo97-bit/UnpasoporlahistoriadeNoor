...
base_dir = r"C:\Users\leoga\Desktop\Noor\Estudio Zotero Por Siglos\Siglo x Rescate 080426"
...
for file in os.listdir(base_dir):
    ...
    if "archaeology" in name or "archaeological" in name or "site" in name or "excavation" in name or "ceramicas" in name or "ceramics" in name or "pottery" in name or "glaze" in name:
        cats.update(["Cultural", "Social", "Económico"]) # Archaeology/Material culture encompasses these

    if "identit" in name or "identidad" in name or "tradition" in name:
        cats.update(["Cultural", "Social"])

    # Fallback if somehow nothing matched (treat as Cultural/Historical mostly)
    if not cats:
        cats.add("Cultural")
